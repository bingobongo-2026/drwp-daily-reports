        ...
        <div class="drwp-dashboard">
          <ul style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;list-style:none;margin:0 0 12px;padding:0;">
            ...
            <li style="background:#fee2e2;padding:10px;border-radius:6px;">
              <div style="color:#991b1b;font-size:.85em;">差し戻し</div>
            ...
            <li style="background:#dcfce7;padding:10px;border-radius:6px;">
              <div style="color:#166534;font-size:.85em;">承認済み</div>
          ...
          <h3 style="margin:12px 0 6px;font-size:.95em;">最近の日報</h3>
          <?php if (empty($recent)): ?>
            <p style="color:#50575e;">まだ日報がありません。</p>
          <?php else: ?>
              ...
              <?php foreach ($recent as $r): ?>
                <li style="padding:6px 0;border-bottom:1px solid #f0f0f1;">
                  <a href="<?php echo esc_url(admin_url('admin.php?page=drwp_report_edit&id=' . (int) $r->id)); ?>">
                    <?php echo esc_html($r->report_date); ?> — <?php echo esc_html($r->public_title ?: '（未設定）'); ?>
                  </a>
          ...
          <p style="margin-top:12px;">
            <a class="button button-primary button-small" href="<?php echo esc_url(admin_url('admin.php?page=drwp_report_edit')); ?>">日報を作成</a>
            <a class="button button-small" href="<?php echo esc_url($list_url); ?>">一覧を開く</a>
          </p>
        </div>
        <?php
    }
