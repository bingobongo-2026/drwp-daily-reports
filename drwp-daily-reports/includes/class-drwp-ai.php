...
class DRWP_AI {
    ...
    // 各 AI 機能が使うシステムプロンプト(AI への指示文)の上書きを
    // まとめて入れる連想配列オプション。キーは prompt_defaults() の
    // スラッグ。未設定・空文字のキーは既定文が使われる。利用者が
    // 設定画面から編集でき、「初期設定に戻す」で該当キーを消せる。
    const OPT_PROMPTS = 'drwp_ai_prompts';
    ...
    /**
     * 各 AI 機能のシステムプロンプト既定値と、設定画面に出すラベル /
     * 補足説明。キー(スラッグ)は system_prompt() / 保存処理で共有する。
     * `default` は各機能のプロンプト組み立て箇所と完全一致させること。
     */
    public static function prompt_defaults() {
        return [
            'briefing' => [
                'label' => __('次回訪問ブリーフィング', 'drwp-daily-reports'),
                'desc'  => __('案件ページの「AI ブリーフィング」。日報から持参物・注意点・提案ポイントを提案します。', 'drwp-daily-reports'),
                'default' =>
                    'あなたは建設・サービス業の案件担当者を支援するアシスタントです。'
            ...
            'draft' => [
                'label' => __('公開記事の下書き生成', 'drwp-daily-reports'),
                'desc'  => __('日報編集ページの「AI で下書きを生成」。⚠ 末尾の出力形式（===TITLE=== などの区切り）を変えると下書きが正しく取り込めなくなります。個人名のイニシャル化・市区町村ルールもここで指示しています。', 'drwp-daily-reports'),
                'default' =>
                    'あなたは建設・サービス業の会社の広報担当ライターです。'
                    . '社内の作業日報をもとに、お客様や一般の読者に向けた公開ブログ記事の下書きを作成します。'
                    . '日報に書かれていない事実は創作しないでください（数値・固有名詞の捏造禁止）。'
                    . "\n\n"
                    . "【個人名の扱い（タイトル・本文の両方に必ず適用）】\n"
                    . "個人名（姓）は必ずイニシャル化してください。「斎藤邸」「田中様」「山田さん」の\n"
                    . "ように姓を含む表記は、姓のローマ字の頭文字1文字（大文字）に置き換えます。\n"
                    . "  例: 斎藤邸 → S様邸 / 田中様 → T様 / 山田さん → Yさん\n"
                    . "会社名・製品名・地名（市区町村など）はイニシャル化しません。\n"
                    . "電話番号や番地までの詳細住所は本文に含めないでください（市区町村までは記載可）。\n\n"
                    . "【タイトルの作り方】\n"
                    . "タイトルは「どこ(市区町村)で・どの現場で・何を・いつ」が一目で分かるよう、次の\n"
                    . "構造を必ず守ってください:\n"
                    . "  [市区町村] [イニシャル化した現場名] で [具体的な作業] （YYYY年M月D日 または M月D日）\n"
                    . "・所在地の市区町村名は、日付より前（タイトル前方）に置いてください。\n"
                    . "・「所在地(市区町村)」が渡された場合はその値を使います。「所在地(住所)」しか\n"
                    . "  渡されていない場合は、そこから市区町村部分だけを取り出して使います。\n"
                    . "・所在地の情報が無い場合は市区町村を省略します（絶対にでっちあげない）。\n"
                    . "例:\n"
                    . "  「静岡市 Y様邸新築工事で上棟・屋根工事を実施しました（2026年6月11日）」\n"
            ...
            'summary' => [
                'label' => __('案件サマリ', 'drwp-daily-reports'),
                'desc'  => __('案件ページの「AI サマリ」。指定期間の日報をまとめて進捗報告を作成します。', 'drwp-daily-reports'),
                'default' =>
                    'あなたは建設・サービス業の現場管理を支援するアシスタントです。'
            ...
            'alerts' => [
                'label' => __('対応必須アラート抽出', 'drwp-daily-reports'),
                'desc'  => __('日報一覧ページの「AI 対応アラート」。特記事項から事務所が対応すべき項目を拾い上げます。', 'drwp-daily-reports'),
                'default' =>
                    'あなたは現場の特記事項を確認する管理者アシスタントです。'
            ...
            'advise' => [
                'label' => __('振り返りアドバイス', 'drwp-daily-reports'),
                'desc'  => __('日報一覧ページの「AI 振り返りアドバイス」。絞り込んだ日報を横断して助言をまとめます。', 'drwp-daily-reports'),
                'default' =>
                    'あなたは建設・サービス業の案件担当を支援するベテランアドバイザーです。'
    ...
    /**
     * 指定機能のシステムプロンプトを返す。利用者が設定画面で上書きして
     * いればその文面、なければ prompt_defaults() の既定文。未知のキーは空。
     */
    public static function system_prompt($key) {
        $defaults = self::prompt_defaults();
        if (!isset($defaults[$key])) return '';
        $overrides = get_option(self::OPT_PROMPTS, []);
        if (is_array($overrides) && isset($overrides[$key])) {
            $custom = trim((string) $overrides[$key]);
            if ($custom !== '') return (string) $overrides[$key];
        }
    ...
    private static function build_briefing_messages($project, $reports) {
        ...
        $system = self::system_prompt('briefing');
    ...
    public static function draft_public_post($report_id) {
        ...
        // 所在地 (タイトルに市区町村を入れるため)。案件に無ければ顧客に
        // フォールバック。市区町村が取れなければ住所全体を渡して AI に
        // 抽出させる。これらはメタ情報なので has_content には数えない。
        $city = '';
        $addr = '';
        if ($project) {
            $city = trim((string) ($project->city ?? ''));
            $addr = trim((string) ($project->address ?? ''));
            if (($city === '' || $addr === '') && !empty($project->customer_id) && class_exists('DRWP_Customer')) {
                $cust = DRWP_Customer::find((int) $project->customer_id);
                if ($cust) {
                    if ($city === '') $city = trim((string) ($cust->city ?? ''));
                    if ($addr === '') $addr = trim((string) ($cust->address ?? ''));
                }
            }
        }
        if ($city !== '') {
        ...
        // 崩れやすいので、デリミタ方式を使う)。プロンプト文面は設定画面
        // から編集可能(既定は prompt_defaults()['draft'])。
        $system = self::system_prompt('draft');
        $user = "【作業日報の内容】\n" . implode("\n", $src);
    ...
    public static function project_summary($project_id, $date_from, $date_to, $period_label = '') {
        ...
        $system = self::system_prompt('summary');
    ...
    public static function extract_alerts($date_from, $date_to, $project_id = 0) {
        ...
        $system = self::system_prompt('alerts');
    ...
    public static function advise_on_reports(array $report_ids) {
        ...
        $system = self::system_prompt('advise');
