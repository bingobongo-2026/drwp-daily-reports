...
**予定の日付変更 (drag-and-drop)**: カレンダー上の予定チップは `draggable=true` で、別の日のセル(`data-date`)にドロップすると `PATCH /plans/{id}` に `planned_date` だけ送って日付を移動する。クリックで「予定→日報テンプレ」が走る既存挙動は残す(ドラッグとクリックは別イベント)。ドロップ先には amber dashed のハイライトが出る。`linked_report_id` 付き(日報と紐づき済)の予定はドラッグ対象外 — 済んだ予定の日付は動かさない。サーバ側でも `DRWP_Plan::can_edit($plan)` が無理な書き込みを弾くので、ドラッグ可視性は飾り、実権限は REST に集約。HTML5 drag-and-drop はモバイルで動かない実装が多いので、現状はデスクトップ向け機能 — モバイルからの日付変更は予定 admin ページの編集モーダルを使う。
