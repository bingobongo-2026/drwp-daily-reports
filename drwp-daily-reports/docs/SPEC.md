...
**ビューモーダルの構造**: 旧の `<table>` + `<h4>` 形式を「ヒーロー(案件名 + ステータスピル) → メタ行(📅 日付 / ⏱ 時刻 / 👤 作成者) → セクション(📝 作業内容 / 💬 特記事項 / 📋 次回予定) → 📷 写真グリッド」のカード型レイアウトに置換。`/reports/{id}` のレスポンスに `project_name` と `author_name` を追加して、追加のリクエストなしで描画できるようにした。
