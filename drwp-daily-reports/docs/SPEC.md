...
[現場作業員のスマホ]
   ...
   ↓ [drwp_report_archive] で日報入力(1 現場ずつ)
...
レビュー・公開ライフサイクルは持たない。フロントアーカイブの月オーバーレイは `DRWP_Plan::for_archive_month()` が供給する。予定編集モーダルの「同日・同案件」日報候補はクライアントが REST `/reports?date_from=...&project_id=...` を直接叩いて出す。
...
| `[drwp_report_archive]` / `[drwp_login_form]` ショートコード | ログイン中の退職者(レース)はデータ/フォームを出さず通知のみ。ログアウト済み+マーカーありは「前回のアカウントは退職状態…」の通知を出しつつ**ログインフォームも併せて表示**(共有端末で別の社員がそのままログインできるようにする — 通知だけ出すとマーカーが切れるまで誰もログインできなくなる) |
...
### 6.1 入力フォーム本体 (`DRWP_Report_Form::render_form`)

`class-drwp-report-form.php`。field worker 向けスマホ向け入力フォーム。これ自体はショートコードではなく、`[drwp_report_archive]` の「日報を書く」モーダル内に埋め込まれる **公開ヘルパー**として呼ばれる。

#### 前提

- 未ログインなら `[drwp_report_archive]` 側でログイン誘導画面に切り替わるので、ここまで到達するのはログイン済みユーザだけ
- `edit_posts` がなければ archive 側で弾かれる

#### 入力フォーム

1 フォーム = 1 現場 = 1 日報。**カード追加機能は無し**(複数現場の日はフォームを連続投稿)。

入力項目: 日付、現場、開始時刻、終了時刻、作業内容、問題点、次回予定、写真。写真は撮影 / 端末選択を兼ねた `<input type="file" accept="image/*" capture="environment" multiple>`。送信時に **写真先行アップロード** → 完了後に `POST /reports` で本体を送る。完了後は緑バナーでメッセージ表示 + 自動スクロール、フォームはリセットされて連続投稿可能(モーダルを閉じてカレンダーに戻りたい場合は archive 側の閉じるボタン)。
...
### 6.2 `[drwp_login_form]` — ログインフォーム(オプション)

`class-drwp-login.php`。

- 未ログイン時: `wp_login_form()` をスタイル付きで描画、リダイレクト先 `redirect_to` のチェーンは:
  1. 呼び出し元から明示的に渡された URL(`render_login_box($url)` 引数)
  2. `?redirect_to=...` クエリが来ていればそれ
  3. それ以外で単一ページ (`is_singular()`) ならそのページ自身
  4. それ以外なら `/wp-admin/admin.php?page=drwp_reports`
- 「パスワードをお忘れですか?」リンクは、設定で `drwp_login_lostpass_page_id` が指定されていればその固定ページへ、未設定なら `wp_lostpassword_url()` (WP 標準)
- ログイン済み時: `<div class="drwp-login-bar">` をドキュメントフロー内に描画。「○○ さんとしてログイン中 [ログアウト]」、CSS で position:fixed なのでページのどこから emit されても画面上部に来る。

`[drwp_report_archive]` 単独で運用する場合、このショートコードは省略可能 — archive 側が同じ `render_login_box()` / `render_logged_in_bar()` を内蔵で呼ぶ。両方を 1 ページに置いてもショートコードは `$bar_rendered` / `$box_rendered` の static フラグで dedup されるので、ログインバー / フォームは 1 つだけ出る。

`[drwp_login_form]` が引き続き必要な典型シーン: 専用ログインページ(日報カレンダーは別ページに置く運用)、`drwp_login_page_id` 設定の固定ページ(退職社員の landing 先)など。
...
### 6.4 `[drwp_report_archive]` — 過去日報の閲覧 + ログインの統合

`class-drwp-report-archive.php`。

ランディングページから「過去にどの作業者がどんな日報を書いたか」を確認できるショートコード。**`[drwp_login_form]` を別ページに分けなくても、これ 1 つでログイン状態に応じた表示分岐が全部回る** ようにエントリポイントで状態判定する:

| ログイン状態 | 表示 |
| --- | --- |
| 退職フラグ持ちで現セッション(レース) | `.drwp-archive-retired`「このアカウントは退職状態のため、ログインできません。」のみ |
| 未ログイン + 退職マーカー | `.drwp-archive-retired` 通知 + ログインフォーム(共有端末対応) |
| 未ログイン | `DRWP_Login::render_login_box(get_permalink())` を呼んでフォームをそのまま埋め込み、`redirect_to` を同じページに張る |
| ログイン済みかつ `edit_posts` 無し | 「閲覧する権限がありません。」 |
| 通常 | 「○○ さんとしてログイン中 / ログアウト」バー + 下記の 3 ビューのうち URL 状態に合致したもの |

ログイン済み時は URL 状態で 3 ビューに分岐:
...
**絞り込みフォーム**: `<details class="drwp-archive-filter-card">` で折りたたみ可能。デフォルトは閉(カレンダーの縦スペースを優先)、`q` / `project` / `status` / `mine` のいずれかが設定済みなら `[open]` で展開する。サマリーに `条件あり` のバッジが出るので閉じていても状態が見える。

**ビューモーダルの構造**: v1.37 以降は wp-admin 日報一覧の「確認」モーダル(`.drwp-page` 系)と同じ紙面風レイアウトに統一 — タイトル帯「作業日報」+ 太枠のメタ表(案件名 / 日付 / 作業時間 / 報告者 / レビュー)+ セクションテーブル(作業内容 / 特記事項 / 次回予定 / 写真)。空欄のセクションは `（記載なし）` の薄色テキストでブロック自体は残す(紙面感を崩さないため)。`/reports/{id}` のレスポンスに `project_name` と `author_name` を追加して、追加のリクエストなしで描画できる。

**インライン編集**: `pending` 状態の日報を開いたモーダルの中で `編集する` ボタン → JS が同じモーダルの body を read-only ビュー → 編集フォームに差し替える(別ページ遷移しない)。保存は `PATCH /reports/{id}`、写真は既存 `POST /upload-photo` を再利用。保存成功で view モードに戻り、最新ペイロードで再描画。`?drwp_id=N&drwp_edit=1` URL での編集ビュー (`render_edit()`) は残してあるので、直接リンクは引き続き有効。
...
を返す。`linked_report_id` がセット済みの予定はグレーアウト表示。グレーアウト(リンク済み)の予定はドラッグ・長押し編集・タップ(日報テンプレ作成)すべて対象外 — 済んだ予定には手を出さない。

**予定チップクリック → 日報をテンプレで作成 + 自動リンク**: リンク済みでない予定チップを押すと「日報を書く」モーダルが開き、`report_date` / `project_id` / `started_at` / `ended_at` が予定からプリフィルされる。フォーム上部に `📋 予定 #N からテンプレを読み込みました（保存するとこの予定は完了になります）` のヒントバナー + 予定のメモが出る。隠しフィールド `linked_plan_id` が日報作成 POST に乗り、`DRWP_REST::create_report` が `link_plan_to_report()` で **その予定の `linked_report_id` に新しい日報 ID をセットし `status` を `completed` に**する。対象は「呼び出し元が編集権限を持つ・まだ未リンク」の予定のみ(他人の予定や済んだ予定は触らない)。監査ログに `plan_linked_to_report` を残す。完了になった予定はカレンダー上でグレーアウトし、以降ドラッグ/長押し編集/タップの対象外になる。
...
**予定チップの 3 経路操作**: タップ / 長押し / ドラッグでそれぞれ別の挙動を持つ:

| 操作 | 挙動 |
| --- | --- |
| タップ(click) | 「予定→日報テンプレ」モーダルが開き、日付・案件・時刻をプリフィル |
| 長押し(touch / mouse 500ms) | 予定編集ダイアログが開き、`planned_date` / `project_id` / `started_at` / `ended_at` / `notes` を編集 → `PATCH /plans/{id}` で保存。担当者(`user_id`)の付け替えは事務所(`edit_others_posts`)のみで select が出る |
| ドラッグ(desktop のみ) | 別の日のセル(`data-date`)にドロップで `PATCH /plans/{id}` に `planned_date` のみ送信 |

長押し検出は touch / mouse 両対応で、500ms 経過前にドラッグが始まったり指を離したりしたらキャンセル(普通のクリック / ドラッグへ自然にフォールバック)。長押し発火後の click イベントは capture フェーズで握り潰して「予定→日報テンプレ」が誤って走るのを防ぐ。

ドロップ先には amber dashed のハイライトが出る。`linked_report_id` 付き(日報と紐づき済)の予定はドラッグも長押し編集も対象外 — 済んだ予定は触らない。サーバ側でも `DRWP_Plan::can_edit($plan)` が無理な書き込みを弾くので、フロントの可視性は飾り、実権限は REST に集約。

カレンダー下に `.drwp-archive-cal-hint` で操作ヒント(「タップで日報作成、長押しで編集、ドラッグ&ドロップで日付を変更」)を控えめに表示してディスカバラビリティを担保。
...
| PATCH | `/plans/{id}` | `edit_posts` + ライセンス + `DRWP_Plan::can_edit($plan)` | 予定 1 件の部分更新。`planned_date` / `project_id` / `user_id` / `started_at` / `ended_at` / `notes` / `status` / `linked_report_id` のうち送られたフィールドだけ書き換え。`project_id` は存在チェック付き(空は案件解除)。`user_id` の変更は `edit_others_posts` が必要(作業員は他社員に振り直せない)。フロントの drag-and-drop / 長押し編集がこの経路 |
...
POST のみ `linked_plan_id`(任意)も受け付ける — 予定チップから作成された日報をその予定に紐づけて完了にする（上記 §6.4 参照、作成後に `link_plan_to_report()` が処理）。
...
| `public/assets/mobile-form.css` | `[drwp_report_archive]` 用 CSS |
| `public/assets/mobile-form.js` | `[drwp_report_archive]` 用 JS(config は wrapper の data 属性から読む) |
...
- **アセット配信経路**: `[drwp_report_archive]` の config を `wp_localize_script` や `wp_add_inline_script` で出すと、ホストのキャッシュ/最適化プラグインに剥がされる事故が観測されている(LiteSpeed Cache, Autoptimize 等)。本プラグインは **wrapper 要素の data 属性に JSON を埋め込む**方式に統一しているので、新規にフロント機能を足すときは同じパターンを推奨する。
