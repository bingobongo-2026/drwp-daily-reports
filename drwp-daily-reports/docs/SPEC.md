...
**予定チップクリック → 日報をテンプレで作成**: リンク済みでない予定チップを押すと「日報を書く」モーダルが開き、`report_date` / `project_id` / `started_at` / `ended_at` が予定からプリフィルされる。フォーム上部に `📋 予定 #N からテンプレを読み込みました` のヒントバナー + 予定のメモが出る。`linked_plan_id` の隠しフィールドも仕込んでいる（将来の自動リンク用、現状は未消費）。
