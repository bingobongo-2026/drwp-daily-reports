...
| 長押し(touch / mouse 500ms) | 予定編集ダイアログが開き、`planned_date` / `started_at` / `ended_at` / `notes` を編集 → `PATCH /plans/{id}` で保存 |
...
| PATCH | `/plans/{id}` | `edit_posts` + ライセンス + `DRWP_Plan::can_edit($plan)` | 予定 1 件の部分更新。`planned_date` / `started_at` / `ended_at` / `notes` / `status` / `linked_report_id` のうち送られたフィールドだけ書き換え。フロントの drag-and-drop 日付変更がこの経路 |
