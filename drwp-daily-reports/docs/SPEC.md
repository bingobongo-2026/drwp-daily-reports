...
- ログイン済み時: `<div class="drwp-login-bar">` を**画面上部固定バー**として描画。「○○ さんとしてログイン中 [ログアウト]」。WP admin bar (32px / モバイル 46px) と共存する

### 6.3 `/wp-login.php` のリダイレクト
...
| `drwp_login_lostpass_page_id` | パスワード再設定リンクの遷移先(未設定なら WP 標準) |
...
`drwp_login_page_id`, `drwp_login_redirect_enabled`, `drwp_login_lostpass_page_id`
...
| `public/assets/login.css` | `[drwp_login_form]` 用 CSS + ログイン状態バー |
