...
<div class="wrap">
  ...
  <?php if (!empty($_GET['saved'])): ?>
    <?php DRWP_Admin::admin_notice('success', __('顧客を保存しました。', 'drwp-daily-reports')); ?>
  <?php endif; ?>
  <?php if (!empty($_GET['error']) && $_GET['error'] === 'missing_name'): ?>
    <?php DRWP_Admin::admin_notice('error', __('顧客名は必須です。', 'drwp-daily-reports')); ?>
  <?php endif; ?>
  ...
  <?php $filter_open = ($filters['search'] !== '' || !empty($filters['group_id'])); ?>
  <details class="drwp-filter" <?php echo $filter_open ? 'open' : ''; ?>>
    ...
    <form method="get" class="drwp-filter-form">
      ...
      <div class="drwp-row">
        ...
        <a class="button-link" href="<?php echo esc_url(admin_url('admin.php?page=drwp_customers')); ?>">
