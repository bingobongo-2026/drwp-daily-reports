...
<div class="wrap">
  ...
  <?php if (!empty($_GET['saved'])): ?>
    <div class="notice notice-success"><p><?php esc_html_e('グループを保存しました。', 'drwp-daily-reports'); ?></p></div>
  <?php endif; ?>
  ...
  <p>
    <button type="button" class="button button-primary" id="drwp-cg-add-btn">
      + <?php esc_html_e('新しいグループを追加', 'drwp-daily-reports'); ?>
    </button>
  </p>
  ...
  <table class="widefat striped" style="margin-top:8px;">
    ...
    <tbody>
      <?php if (empty($groups)): ?>
        <tr><td colspan="7"><?php esc_html_e('まだグループがありません。', 'drwp-daily-reports'); ?></td></tr>
  ...
  <dialog id="drwp-cg-dialog" class="drwp-cg-modal">
    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
      ...
      <div class="drwp-cg-modal-header">
        <h2 id="drwp-cg-title"><?php esc_html_e('新しいグループを追加', 'drwp-daily-reports'); ?></h2>
...
(function () {
  ...
  var addTitle  = <?php echo wp_json_encode(__('新しいグループを追加', 'drwp-daily-reports')); ?>;
  var editTitle = <?php echo wp_json_encode(__('グループを編集', 'drwp-daily-reports')); ?>;
