...
- 日報マン プラグイン(`drwp-daily-reports`): **1.77.1**
- テーマ jijipom: **1.18.0**
...
- 日報: 編集UIが3系統に分裂(一覧モーダル/隠しページ report_edit/フロント)。
  A/B のデータ消失(#282)は解消済みだが、3系統の統合自体は未着手。
- 日報: モーダルの CSS が複数ファイルにコピペのまま。ダッシュボードはインライン
  style が約30個(フィルタパネルCSSの集約=#284 は完了済み)。
- 日報: 予定と日報の紐付けが ID 手入力。AI実行の進捗表示なし。
- license-server: 一覧のページネーション無し(件数増で全行ロード)。設定タブが
  URLに反映されない。監査ログ30件固定。管理トークンのDB平文保存(ハッシュ化は
  READMEの「管理者が現行トークンを閲覧できる」前提と要調整)。バックアップzip
  無暗号化。(uvicorn --proxy-headers・.env.example は #285 で対応済み)。
