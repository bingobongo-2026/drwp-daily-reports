...
- 日報マン プラグイン(`drwp-daily-reports`): **1.79.9**
- テーマ jijipom: **1.19.2**
...
- 日報 1.79.3〜1.79.9: 全体デバッグ (2026-09) で確認した🔴7件を順に修正。
  ①ライセンス有効判定を検証済みチェック7日以内に限定 (fail-open解消)
  ②RESTのPATCHで明示的nullをクリア指示として許可 + 編集モーダルに完了案件
  を「（完了）」付きで表示 ③予定の新規作成で担当者・状態・紐付け日報を受付
  ④予約公開は未来のscheduled_at必須 (全変換経路) ⑤アーカイブ済み日報の
  記事化ブロック (一覧除外+REST 409+sync_postガード) ⑥スマホの「自分のみ」
  チェック表示を実効スコープに一致 ⑦記事化権限をcan_edit_one流用から
  「publish_posts+自分の承認済み」判定へ分離、死にコード撤去 (convert_single /
  DRWP_Reviewのadmin-postハンドラ / bulk 3分岐 / 公開プレビューページ) と
  CSV系修正 (日報CSVにアーカイブ選択を引き継ぐ / 監査CSVの検索キー s 付替 /
  予定一覧の空メッセージ判定から orderby/order 除外)。

**未対応**: デバッグ報告の🟠中優先はまだ残っている (post_status のPATCH
検証なし / CSV数式インジェクション / ゴミ箱復活 / before_after強制分割 /
サムネイル無し写真の記事化欠落 / 自分への通知 / 管理URLとロックダウン /
expires_at未使用 / 予定完了の見えづらさ / CSV再取込の担当者巻き戻り /
AIエラーの生表示 など)。着手時はデバッグ報告の一覧を参照。
