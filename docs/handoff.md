...
- 日報マン プラグイン(`drwp-daily-reports`): **1.78.0**
- テーマ jijipom: **1.18.0**
...
**未対応(残りは以下の2つ。いずれも着手前に判断/準備が必要)**:
- 日報: 編集UIが3系統に分裂(一覧モーダル/隠しページ report_edit/フロント)。
  A/B のデータ消失(#282)は解消済み。統合には「どのUIに寄せるか」の製品判断が
  必要なため保留。
- 日報: モーダルの CSS が複数ファイルにコピペのまま。ダッシュボードはインライン
  style が約30個(フィルタパネルCSSの集約=#284 は完了済み)。値が微妙に不一致の
  まま機械的に統合すると見た目が変わり得るため、スクリーンショット比較を
  しながらの作業を推奨。
