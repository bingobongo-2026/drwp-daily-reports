...
- license-server: 管理トークンのDB平文保存(ハッシュ化は READMEの「管理者が
  現行トークンを閲覧できる」前提と要調整)。バックアップzip 無暗号化。
  (uvicorn --proxy-headers・.env.example は #285、一覧ページネーション・
  設定タブURL反映・監査ログ件数切替は #287 で対応済み)。
