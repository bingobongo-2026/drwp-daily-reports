...
- 日報マン プラグイン(`drwp-daily-reports`): **1.79.1**
- テーマ jijipom: **1.18.0**
...
**未対応(残り)**:
- 日報: モーダルCSS集約の第2弾。案件(projects-page)・顧客(customers-page)・
  記事一覧(articles-list)の3ページが未移行(それぞれ固有CSSが多め。articles は
  drwp-modal と同名クラスをインラインで重複定義しているので置換時に注意)。
  移行方式は第1弾と同じ「共通クラス併記+シェルCSS削除+実質差分のみ残す」。
