...
「日報マン」プラグイン用のスタンドアロンライセンスサーバー（FastAPI）。
...
トークンは salted PBKDF2 ハッシュとして保存され、平文は DB に残りません（DB やバックアップ zip が漏れてもトークンはそのまま使えません）。そのため**保存後にトークンを確認する手段はありません** — 保存前に必ずパスワードマネージャ等に控えてください。忘れた場合は環境変数 `DRWP_ADMIN_TOKEN` でブートストラップし直すか、この画面の「DB の値を削除」で環境変数へフォールバックさせて再設定します。旧バージョンで平文保存されたトークンは、起動時（および旧バックアップの復元時）に自動でハッシュへ移行されます。
...
- `data.sqlite3` — ライセンス DB（settings テーブル含む = ユーザー名 / トークンのハッシュも復元される）

月 1 回程度ダウンロードして手元に保管しておけば、コンテナごと消えた場合でも復元できます。復元は同名ファイルを上書きする形で行われ、復元後は管理 UI 上で公開鍵が想定通りであることを確認してください。

環境変数 `DRWP_BACKUP_PASSPHRASE` を設定すると、バックアップはパスフレーズから導出した鍵（PBKDF2 → Fernet/AES）で暗号化された `.zip.enc` として払い出されます。zip には署名秘密鍵とライセンス DB がそのまま入るため、クラウドストレージ等の第三者が触れ得る場所に保管する場合は設定を推奨します。復元には取得時と同じパスフレーズが必要です（暗号化前の古い平文 zip もそのまま復元できます）。
